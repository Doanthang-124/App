...
{
  Serial.begin(115200);
  WiFi.begin("Hien Dung","chicungduoc");  // add ssid and password here
  pinMode(5, OUTPUT);
...
void loop()
{
  ...
  if (data=="on") 
  {
    int res=sendmessage("ON");
    digitalWrite(5, HIGH);
  }
  if (data=="off") 
  {
    int res=sendmessage("OFF");
    digitalWrite(5, LOW);
  }
   if (data=="blink") {
    int res=sendmessage("BLINK");
    digitalWrite(5, HIGH);
    delay(100);
    digitalWrite(5, LOW);
    delay(100);
...
String receivelastmessage()
{
  ...
  {
    HTTPClient http;
    String url="http://qindemo.000webhostapp.com/datastorage.txt";
    http.begin(url);
...
int sendmessage(String d)
{
  ...
  {
    HTTPClient http;
    String url="http://qindemo.000webhostapp.com/readfile.php?data="+d;
    http.begin(url);
    http.addHeader("Content-Type","text/plain");
    int httpCode=http.GET();
